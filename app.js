...
const convertPlayerObObjectToResponseObject = (db0bject) => {
  return {
    playerId: db0bject.player_id,

    playerName: db0bject.player_name,
  };
};

const convertMatchDetailsDbObjectToResponseObject = (db0bject) => {
  //convertMatchDetailsObObjectToResponseObject
  return {
    matchId: db0bject.match_id,
    match: db0bject.match,
    year: db0bject.year,
  };
};
...
player_id= ${playerId};`;
  ...
  response.send(convertPlayerObObjectToResponseObject(player));
});
...
*
FROM player_match_score
NATURAL JOIN player_details WHERE
